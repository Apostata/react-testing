...
describe('App component', ()=>{
    ...
    describe('Render', ()=>{
        ...
        it('Should render Congrats component when success is true', ()=>{
            initialState = { success: true, secretWord: 'party' }
            wrapper = setup(initialState).dive().dive();
    ...
    describe('Redux props', ()=>{
        ...
        context('With guessedWords state', () =>{
            const guessedWord = { guessedWord:'train', letterMatchCount: 3}
            const initialState = { success: false, guessedWords:[guessedWord], secretWord:'party' };

            it('Should render TotalGuesses component if there are guessed words', ()=>{
                const wrapper = setup(initialState).dive().dive();
                const element = findTestAttr(wrapper, 'app-total-guesses');
                expect(element.length).toBe(1);
            })
        });
